...
//add at 2026-02-12 by ocean
static void *keylog_open_file (const SSL *ssl)
{
    ...
    id_str[i * 2] = '\0';
    // 使用环境变量或默认 /tmp
    const char *logdir = getenv("SSLKEYLOGFILE");
    if (!logdir)
        logdir = "/tmp/quic_sslkey.log";

    //sz = snprintf(path, sizeof(path), "/secret_dir/%s.keys", id_str);
    //sz = snprintf(path, sizeof(path), "%s/%s.keys", logdir, id_str);
    sz = snprintf(path, sizeof(path), "%s", logdir);
    if ((size_t) sz >= sizeof(path))
    {
        LSQ_WARN("########WARN: %s: file too long", __func__);
        return NULL;
    }
    fh = fopen(path, "ab");
    if (!fh)
        LSQ_WARN("########WARN: could not open %s for appending: %s", path, strerror(errno));
    else
        LSQ_INFO("########INFO:sslkeylog=%s,key=%s", path,id_str);
...
{
    FILE *file = keylog_open_file(ssl);
    if (file)
    {
        fputs(line, file);
        fputs("\n", file);
    ...
    LSQ_INFO("########INFO:sslkeylog-line=%s", line);
...
main (int argc, char **argv)
{
    ...
    //add set_keylog_callback() at 2026-02-12 by ocean   
